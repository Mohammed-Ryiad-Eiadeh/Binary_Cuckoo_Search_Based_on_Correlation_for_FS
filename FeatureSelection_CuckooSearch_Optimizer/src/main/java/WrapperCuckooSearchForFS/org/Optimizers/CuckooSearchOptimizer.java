...
public  final class CuckooSearchOptimizer implements FeatureSelector<Label> {
    ...
    public SelectedFeatureSet select(Dataset<Label> dataset) {
        ...
        List<CuckooSearchFeatureSet> subSet_fScores = Arrays.stream(setOfSolutions).
                map(setOfSolution -> new CuckooSearchFeatureSet(setOfSolution,
                        evaluateSolution(this, trainer,dataset, FMap, setOfSolution))).
                sorted(Comparator.comparing(CuckooSearchFeatureSet::score).reversed()).collect(Collectors.toList());
            ...
            for (int solution = 0; solution < populationSize; solution++) {
                ...
                if (rng.nextDouble() < worstNestProbability) {
                    ...
                    for (int j = 0; j < setOfSolutions[subSet.get()].length; j++) {
                        double solutionBits = setOfSolutions[subSet.get()][j];
                        double randomSolution1Bits = setOfSolutions[r1][j];
                        double randomSolution2Bits = setOfSolutions[r2][j];
                        evolvedSolution[j] = (int) transferFunction.
                                applyAsDouble( solutionBits + delta * (randomSolution1Bits - randomSolution2Bits));
                    }
                ...
                int[] jayaSolution = jayaOperator(setOfSolutions[subSet.get()],
                        subSet_fScores.get(0).subSet(),
                        subSet_fScores.get(subSet_fScores.size() - 1).subSet());
                keepBestAfterEvaluation(dataset, trainer, FMap, jayaSolution, setOfSolutions[subSet.get()]);
            }
            Arrays.stream(setOfSolutions).
                    map(subSet -> new CuckooSearchFeatureSet(subSet, evaluateSolution(this, trainer, dataset, FMap, subSet)))
    ...
    private  <T extends FeatureSelector<Label>> double evaluateSolution(T optimizer, Trainer<Label> trainer, Dataset<Label> dataset, ImmutableFeatureMap Fmap, int[] solution) {
        ...
        int subsetSize = selectedFeatureDataset.getSelectedFeatures().size();
        int datasetSize = Fmap.size();
        return avgAccuracy + 0.001 * (1 - ((double) subsetSize / datasetSize));
    }
    ...
    private void keepBestAfterEvaluation(Dataset<Label> dataset, Trainer<Label> trainer, ImmutableFeatureMap FMap, int[] alteredSolution, int[] oldSolution) {
        double scoreOfSolution = evaluateSolution(this, trainer, dataset, FMap, alteredSolution);
        double scoreOfModifiedSolution = evaluateSolution(this, trainer, dataset, FMap, oldSolution);
    ...
    private int[] mutation(int[] currentSolution) {
        return Arrays.stream(currentSolution).
                map(x -> ThreadLocalRandom.current().nextDouble() < mutationRate ? 1 - x : x).
                toArray();
    }
    ...
    private int[] jayaOperator(int[] currentSolution, int[] currentBest, int[] currentWorst) {
        ...
        Arrays.setAll(newSolution, i -> (int) transferFunction.applyAsDouble(currentSolution[i] + new Random().nextDouble() *
